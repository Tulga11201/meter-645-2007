...
void Check_Neg_Curr(void)
{
  ...
  if(Read_Event_Status(ID_EVENT_VOLT_NEG_SEQ))  //逆相序了
  {
...
void Check_Volt_Curr_Seq(void)
{
  ...
  //电压逆相序，还需判定最小电压  
  if((FP32S)Get_Min_Num(Measu_InstantData_ToPub_PUCK.Volt.A,Measu_InstantData_ToPub_PUCK.Volt.B,Measu_InstantData_ToPub_PUCK.Volt.C)<=UNIT_V*Get_Un()*0.6)  //最小电压需>=60％Un
  {
...
{
  if(Get_Sys_Status()!=SYS_NORMAL || UP_COVER_STATUS EQ 0)    //VCC
  {
    Clr_Event_Instant(ID_EVENT_UP_COVER_OPEN);
    return ;
  }
  
  if(UP_COVER_STATUS)   //UP_COVER_STATUS,
  {
